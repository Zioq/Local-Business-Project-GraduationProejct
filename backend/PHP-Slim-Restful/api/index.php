...
function login() {
    ...
    try {
        print_r($data);
        print_r($data->passcode);
        $db = getDB();
        $userData ='';
        //$sql = "SELECT user_id, name, email, username FROM users WHERE (username=:username or email=:username) and password=:password ";
        $sql = "SELECT employeeId,employeePasscode FROM EmployeeAccount WHERE (employeePasscode=:employeePasscode) and employeePasscodePassword=:employeePasscodePassword";
        $stmt = $db->prepare($sql);

        $stmt->bindParam("employeePasscode", $data->passcode, PDO::PARAM_STR);
        $employeePasscodePassword=hash('sha256',$data->passcodePassword);
        $stmt->bindParam("employeePasscodePassword", $employeePasscodePassword, PDO::PARAM_STR);
        $stmt->execute();
        ...
        if(!empty($userData))
        {
            ...
            $userData->token = apiToken($employeeId);
        }
